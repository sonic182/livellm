defmodule Livellm.ChatsTest do
  use Livellm.DataCase
  ...
  describe "chats" do
    ...
    import Livellm.ChatsFixtures

    @invalid_attrs %{title: nil, model: nil}
    ...
    test "update_chat/2 with valid data updates the chat" do
      chat = chat_fixture()
      update_attrs = %{title: "some updated title", model: "some updated model"}

      assert {:ok, %Chat{} = chat} = Chats.update_chat(chat, update_attrs)
      assert chat.title == "some updated title"
      assert chat.model == "some updated model"
    end
